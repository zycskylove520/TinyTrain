...
class FaceRecognitionModel(BaseModel):
    ...
    def __init__(self, config_manager: ConfigManager, device, *args, **kwargs):
        ...
        Args:
            config_manager (ConfigManager): 全局配置管理器，需包含
            ...
            *args, **kwargs: 预留扩展字段，透传给 BaseModel。
        """
    ...
    def custom_parse_model(self, layer, module_info):
        ...
        if name == "MobileFaceNet":
            # MobileFaceNet：按尺度索引线性扩增通道，GDC 层固定输出 embedding_size。
            ...
            # ResFaceNet：四个 ResNetLayer 重复次数完全由 scale 决定；GDC 同 MobileFaceNet。
            if scale == "n":
